...
void cross_validation_fuzzy(const struct problem *prob, const struct parameter *param, int nr_fold, double *target)
{
        int i;
        ...
        for(i=0;i<nr_fold;i++)
        {
                ...
                double mult = 1;
                if(submodel->label[0] < 0){
                    mult = -1;
                }
                for(j=begin;j<end;j++){
                        predict_values(submodel,prob->x[perm[j]], &(target[perm[j]]));
                        target[perm[j]] *= mult;
                }
...
int save_model(const char *model_file_name, const struct model *model_)
{
	...
	for(i=0; i<w_size; i++)
	{
		int j;
		for(j=0; j<nr_w; j++)
            fprintf(fp, "%.64g ", model_->w[i*nr_w+j]);
		fprintf(fp, "\n");
	}

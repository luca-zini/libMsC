...
void svm_cross_validation_fuzzy(const svm_problem *prob, const svm_parameter *param, int nr_fold, double *target)
{
    int i;
    int *fold_start;
    int l = prob->l;
    ...
    for(i=0;i<nr_fold;i++)
    {
           ...
           (param->svm_type == C_SVC || param->svm_type == NU_SVC))
        {
            ...
            for(j=begin;j<end;j++)
#ifdef _DENSE_REP
                target[perm[j]] = svm_predict_fuzzy(submodel,prob->x+perm[j]);
#else
                target[perm[j]] = svm_predict_fuzzy(submodel,prob->x[perm[j]]);
#endif
        svm_free_and_destroy_model(&submodel);
...
double svm_predict_fuzzy(const svm_model *model, const svm_node *x)
{
    int nr_class = model->nr_class;
    double *dec_values;
    if(model->param.svm_type == ONE_CLASS ||
       ...
       model->param.svm_type == NU_SVR)
        ...
        dec_values = Malloc(double, nr_class*(nr_class-1)/2);
    double pred_result = svm_predict_values(model, x, dec_values);
    pred_result = dec_values[0]*model->label[0];
    free(dec_values);
    return pred_result;
}
